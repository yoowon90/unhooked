...
**IMPORTANT — set `SKIP_DB_CREATE_ALL=1` on every `db migrate` / `db upgrade` command** (both envs). `create_app()` runs `db.create_all()` at import, which silently creates any new tables before autogenerate compares — yielding empty migrations. Also: autogenerate re-proposes `drop_table('favorite_brand')` (legacy table, no model) every time; hand-remove it from generated scripts.

Prod schema for the savings-ledger feature was applied 2026-07-05 (`fa869c032f76` in `migrations_prod`) — the schema exists in BOTH DBs, but the feature is **dev-only** via `SAVINGS_FEATURE_ENABLED` (True in DevelopmentConfig, False in ProductionConfig). Historical wart: the three ledger tables reached prod via a stray `create_all()` before that migration, so they predate the recorded revision.
...
- **Shopping Habits Score**: 0-100 metric on `/home` reflecting purchase impulsiveness vs. unhook patience over a rolling 90-day window. Criteria, formula, and tier thresholds are documented in `docs/shopping_habits_score.md`; tweak constants via `SCORE_WEIGHTS` / `SCORE_TIERS` at the top of `website/reports.py`. v3 also scores the post-purchase savings decision (`moved` +3 / `declined` −3 / never-prompted 0).
- **Savings ledger** (fintech learning feature, **dev-only**: gated by `SAVINGS_FEATURE_ENABLED` in config.py — off in prod, where the interstitial, Savings column, Bank connector, and all Plaid/reconcile routes are hidden/404; the score's savings signal contributes 0 there since no decisions can exist): marking an item purchased shows an interstitial — "Move $X to savings?" with an editable amount. "Yes" posts a double-entry transaction to an internal ledger and originates a real ACH debit via **Plaid Transfer (sandbox only — keep it there)**. Files: `website/ledger.py` (double-entry core: integer cents, sum-to-zero enforced in Python, balances derived never stored, append-only — mistakes fixed by reversal), `website/purchases.py` (shared mark-purchased + decision recording, used by web AND API so the paths can't drift), `website/plaid_client.py` (raw-HTTP Plaid client + sandbox helpers), `website/crypto.py` (Fernet encryption for Plaid tokens; key = `TOKEN_ENCRYPTION_KEY` in .env), `website/transfers.py` (rail bridge; origination failures never un-post ledger intent; idempotency keys derived from txn id + created_at), `website/reconciliation.py` (drains `/transfer/event/sync` from a persisted `SyncCursor`; settles/fails txns, handles post-settlement ACH returns via reversal + `returned` status, flags unmatched transfers). Reconciliation runs opportunistically on purchased-list loads (guarded), via the Settings "Sync transfers" button, and `POST /api/v1/transfers/reconcile`. Tests: `python -m pytest tests/` (in-memory SQLite, no network; Plaid mocked). Plaid env keys: `PLAID_CLIENT_ID`, `PLAID_SANDBOX_SECRET`.
